...
- Integrasi data nyata: impor CSV/OFX/API, auto-kategorisasi berbasis rules/ML sederhana, deduplikasi transaksi, dan refresh saldo/riwayat akun secara periodik. [DONE: Backend lengkap + UI upload + scheduled sync + account linking + per-user scoping/validation + dedup per akun + CSV endpoint fix]
...
1) PRIORITAS TINGGI
   ...
   - Integrasi data: Impor CSV/OFX/API, dedup, auto-kategori, refresh saldo/riwayat akun terjadwal. [DONE: CSV/OFX/API alur + dedup per akun + mapping akun + per-user validation + scheduled sync + CSV upload endpoint]
...
2) PRIORITAS MENENGAH
   ...
   - Observabilitas produk: Event tracking dasar (tanpa PII), logging terstruktur, alert kegagalan integrasi. [DONE: import log per user/integration + status success/failed + context sync + event logs w/ source/ip/user agent + controller hooks]
