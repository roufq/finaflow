                ...
                <div class="card-body">
                    @forelse($news as $article)
                        <div class="mb-3">
                            ...
                            <small class="text-muted">{{ $article->source ?? __('education.news.source') }} · {{ optional($article->published_at)->diffForHumans() }}</small>
                            <p class="text-muted small mb-0">{{ \Illuminate\Support\Str::limit(strip_tags($article->content), 80) }}</p>
                        </div>
                        @if(!$loop->last)
                            <hr>
                        @endif
