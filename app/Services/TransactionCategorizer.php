...
class TransactionCategorizer
{
    ...
    public function guessCategoryId(string $description, float $amount, array $transactionData = []): ?int
    {
        ...
        if (!empty($categoryScores)) {
            ...
            if ($categoryScores[$topCategory] > 0) {
                return $this->findCategoryIdBySlug($topCategory);
            }
        }

        // Fallback to amount-based heuristics
        return $this->guessByAmount($amount);
    }
     ...
     */
    private function guessByAmount(float $amount): ?int
    {
        // Large amounts are likely salary or transfers
        if ($amount >= 5000000) {
            return $this->findCategoryIdBySlug('salary') ?? $this->findCategoryIdBySlug('transfer');
        }

        // Medium amounts might be utilities or shopping
        if ($amount >= 500000) {
            return $this->findCategoryIdBySlug('utilities') ?? $this->findCategoryIdBySlug('shopping');
        }

        // Small amounts could be food or transport
        if ($amount <= 50000) {
            return $this->findCategoryIdBySlug('food_dining') ?? $this->findCategoryIdBySlug('transport');
        }
