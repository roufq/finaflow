...
<div class="max-w-4xl mx-auto space-y-10">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-slate-900">New Configuration</h1>
            <p class="text-sm font-medium text-slate-500">Define multi-currency settings and regional parameters.</p>
        </div>
        ...
        <form action="{{ route('settings.store') }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                <div class="space-y-2">
                    ...
                    <input type="text" id="tags" name="tags" value="{{ old('tags') }}" placeholder="e.g. Global Savings, Business USD" required
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="currency_symbol" name="currency_symbol" value="{{ old('currency_symbol', 'Rp') }}" placeholder="Rp, $, €, etc." required
                           class="w-full rounded-2xl border-none bg-slate-50 px-5 py-3.5 text-sm font-bold text-slate-900 ring-1 ring-slate-200 transition-all focus:bg-white focus:ring-4 focus:ring-primary-500/10">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="number" step="0.000001" id="exchange_rate" name="exchange_rate" value="{{ old('exchange_rate', '1.0') }}" required
                    ...
                    <p class="text-[10px] text-slate-400 font-medium ml-1 italic">Relative to your primary operational currency (1.0 = Base).</p>
                </div>
                <div class="space-y-2">
            ...
            <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <select id="risk_profile" name="risk_profile"
                            ...
                            <option value="conservative" {{ old('risk_profile') === 'conservative' ? 'selected' : '' }}>Conservative (Safety First)</option>
                            <option value="balanced" {{ old('risk_profile') === 'balanced' ? 'selected' : '' }}>Balanced (Moderate Growth)</option>
                            <option value="aggressive" {{ old('risk_profile') === 'aggressive' ? 'selected' : '' }}>Aggressive (Max Yield)</option>
                        </select>
                        <div class="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-slate-400">
                            <i class="fas fa-chevron-down text-[10px]"></i>
                        </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="number" id="credit_score" name="credit_score" value="{{ old('credit_score') }}" min="300" max="900" placeholder="e.g. 750"
                               class="w-full rounded-2xl border-none bg-slate-50 pl-12 pr-5 py-3.5 text-sm font-bold text-slate-900 ring-1 ring-slate-200 transition-all focus:bg-white focus:ring-4 focus:ring-primary-500/10">
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 ring-1 ring-slate-200">
                ...
                <label class="relative inline-flex cursor-pointer items-center">
                    <input type="checkbox" id="is_default" name="is_default" value="1" {{ old('is_default') ? 'checked' : '' }} class="peer sr-only">
