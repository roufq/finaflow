...
<div class="max-w-4xl mx-auto space-y-10">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-slate-900">Modify Transfer</h1>
            <p class="text-sm font-medium text-slate-500">Adjust the parameters for this internal liquidity shift.</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-10 lg:grid-cols-12">
                ...
                <form action="{{ route('transfers.update', $transfer) }}" method="POST" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                        <div class="space-y-2">
                            ...
                            <div class="relative group">
                                ...
                                <input type="number" step="0.01" min="0.01" id="amount" name="amount" value="{{ old('amount', $transfer->amount) }}" placeholder="0.00" required
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative group">
                                ...
                                <input type="number" step="0.01" min="0" id="fee" name="fee" value="{{ old('fee', $transfer->fee) }}" placeholder="0.00"
                    ...
                    <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                        <div class="space-y-2">
                            ...
                            <input type="date" id="transfer_date" name="transfer_date" value="{{ old('transfer_date', $transfer->transfer_date->format('Y-m-d')) }}" required
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" id="reference_number" name="reference_number" value="{{ old('reference_number', $transfer->reference_number) }}" placeholder="e.g. TRX-99827..."
                                   class="w-full rounded-2xl border-none bg-slate-50 px-5 py-3.5 text-sm font-bold text-slate-900 ring-1 ring-slate-200 transition-all focus:bg-white focus:ring-4 focus:ring-primary-500/10">
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="description" name="description" rows="3" placeholder="Contextual notes for this capital movement..."
                                  class="w-full rounded-2xl border-none bg-slate-50 px-5 py-4 text-sm font-medium text-slate-900 ring-1 ring-slate-200 transition-all placeholder:text-slate-300 focus:bg-white focus:ring-4 focus:ring-primary-500/10">{{ old('description', $transfer->description) }}</textarea>
                    </div>
        ...
        <div class="lg:col-span-4 space-y-8">
            <div id="transfers_summary" class="rounded-3xl bg-slate-900 p-8 text-white shadow-premium ring-1 ring-white/10">
                ...
                <div class="space-y-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0">
                            ...
                            <p id="summary_from" class="truncate text-xs font-black text-white">{{ $transfer->fromAccount->name }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0">
                            ...
                            <p id="summary_to" class="truncate text-xs font-black text-white">{{ $transfer->toAccount->name }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            ...
                            <span id="summary_amount" class="text-xs font-black text-white">Rp {{ number_format($transfer->amount, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-[9px] font-bold text-slate-400 uppercase">Friction</span>
                            <span id="summary_fee" class="text-xs font-bold text-rose-400">Rp {{ number_format($transfer->fee, 0, ',', '.') }}</span>
                        </div>
                        <div class="pt-3 border-t border-white/5 flex items-center justify-between">
                            <span class="text-[10px] font-black text-primary-400 uppercase tracking-widest">Total Debit</span>
                            <span id="summary_total" class="text-lg font-black text-white tracking-tighter">Rp {{ number_format($transfer->total_amount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Caution Card -->
            <div class="rounded-3xl bg-amber-50 p-8 shadow-premium ring-1 ring-amber-100 flex flex-col items-center text-center gap-4">
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-amber-500 shadow-sm border border-amber-100">
                    <i class="fas fa-exclamation-circle text-xl"></i>
                </div>
                <div>
                    <h4 class="text-xs font-black text-amber-900 uppercase tracking-widest mb-1">State Modification</h4>
                    <p class="text-[10px] font-medium text-amber-700 leading-relaxed">
                        Modifying a transfer record will recalibrate the ledger history for both associated accounts. Ensure the reference parameters are verified.
                    </p>
                </div>
            </div>
        </div>
